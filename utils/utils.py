...
def chr_except_space(x, replace_a, with_b):
    # x: int, replace_a: int, with_b: int
    return chr(with_b) if x == replace_a else chr(x)


def ord_except_space(x, replace_b, with_a):
    # x: char, replace_b: char, with_a: char
    return ord(with_a) if x == replace_b else ord(x)
...
def get_feat(data_set, model, device):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(data_set):
            _, h = model(batch["x"].to(device))
            data.append(to_numpy(h))
